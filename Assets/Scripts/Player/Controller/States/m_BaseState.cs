...
public class m_BaseState
{
    ...
    public virtual void Entry() { Debug.Log(GetState().ToString()); 
    }
    ...
    {
        if (Physics.BoxCast(agent.transform.position, Vector3.one * .2f, Vector3.down, out RaycastHit _hit, Quaternion.identity, 1.3f, agent.agentSettings.groundMasks))
        {
            ...
            return angle <= agent.agentSettings.maxSlopeAngle;
        }
    ...
    {
        if (Physics.Raycast((agent.transform.position - Vector3.up) + Vector3.up * 0.01f, _moveDirection, out RaycastHit _lowerhit, agent.GetComponent<CapsuleCollider>().radius + .6f, agent.agentSettings.groundMasks))
        {
            if (Vector3.Dot(Vector3.up, _lowerhit.normal) != 0.0f) return;

            if (Physics.Raycast(_lowerhit.point - _lowerhit.normal * .15f + Vector3.up, Vector3.down, out RaycastHit _upperhit, 1f, agent.agentSettings.groundMasks))
            {
                if(Mathf.Abs(_upperhit.point.y - _lowerhit.point.y) <= agent.agentSettings.maxStepHeight)
                    agent.GetRigidbody().position -= new Vector3(0f, -agent.agentSettings.smoothStep * Time.fixedDeltaTime, 0f);
            }
        }
    ...
    {        
        return Physics.OverlapSphere(agent.transform.position + Vector3.down, 0.15f, agent.agentSettings.groundMasks).Length > 0;
    }

    protected void Jump()
    {
        if(IsGrounded() && GetState() != StateID.Crouch)
